...
class SearchItemUseCaseTest {
    ...
    fun searchComplete() {
        ...
        ).willReturn(Single.just(Arrays.asList(createItem())))

        val test =
            searchItemUseCase.createObservable(SearchItemUseCase.Params(anyString(), anyInt()))
                .test()
    ...
    fun searchReturnData() {
        ...
        given(itemRepository.searchItems(params.query, params.pageNumber)).willReturn(
            Single.just(
                items
            )
        )
